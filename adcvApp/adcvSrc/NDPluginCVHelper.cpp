...
void NDPluginCVHelper::print_cv_error(Exception& e, const char* functionName) {
    ...
    sprintf(buff, "OpenCV Error in function %s: %s code: %d file: %s\n", functionName,
            e.err.c_str(), e.code, e.file.c_str());
...
double NDPluginCVHelper::compute_rect_distance(Rect r1, Rect r2) {
    ...
    if (r1.x > r2.x + r2.width)
        xDist = r1.x - (r2.x + r2.width);
    else if (r2.x > r1.x + r1.width)
        xDist = r2.x - (r1.x + r1.width);

    if (r1.y > r2.y + r2.height)
        yDist = r1.y - (r2.y + r2.height);
    else if (r2.y > r1.y + r1.height)
        yDist = r2.y - (r1.y + r1.height);
 ...
 * Function that thresholds an image based on a certain pixel value. First, the image is converted
 * to grayscale. RGB images cannot be thresholded. For each pixel, if the grayscale value is larger
 * than the threshold, set it to white, otherwise set it to black. Creates a binary image
 *
 * @inCount     -> 3
 ...
 * Function for laplacian-based edge detection. First, the image is converted to grayscale if it is
 * not already. Next, the image is blurred using a gaussian kernel to emphasize edges. Then a
 * laplacian kernel runs over the images assigning a 'sharpness' value to each pixel. The sharpest
 * values are hard edges from black to white.
 *
 * @inCount     -> 1
 ...
 *
 * @inCount     -> 4
 * @inFormat    -> [Gaussian blurr (Int), Laplacian kernel size (Int), Laplacian scale (Int),
 * Laplacian delat (Int) ]
 *
 * @outCount    -> 0
 * @outFormat   -> N/A
 ...
 * Function for canny-based edge detection. First, we ensure that the image is grayscale. Then, the
 * image is blurred, so that only strong edges remain. Then, a threshold is applied to the image in
 * order to further reinforce strong edges. Finally, the canny algorithm is applied to the image,
 * and the edges are displayed. The function outputs some information based on the detected edges
 * that can assist with object detection/identification: the top, bottom, left, and right pixels are
 * the min and max X and Y pixel values that appear on one of the edges. The horizontal and vertical
 * size and center give you the spacing between these min and max values and their midpoint.
 *
 * @inCount     -> 3
 * @inFormat    -> [Threshold value (Int), Threshold ratio (Int), Blur degree (Int), Kernel Size
 * (Int)]
 *
 * @outCount    -> 8
 * @outFormat   -> [Horizontal Center, Horizontal Size, Vertical Center, Vertical Size, Top Pixel,
 * Bottom Pixel, Left Pixel, Right Pixel]
 */
 ...
 * Function that allows the user to take consecutive images recieved from area detector and subtract
 * them in pairs. Reads first image into memory, then waits for second one, when it receives the
 * second one, subtract them.
 *
 * @inCount     -> 0
 * @inFormat    -> N/A
 *
 ...
 * @outFormat   -> N/A
 */
ADCVStatus_t NDPluginCVHelper::subtract_consecutive_images(Mat& img, double* inputs,
                                                           double* outputs) {
...
/**
 * WRAPPER  ->  VideoRecord
 * This function uses the opencv_video and opencv_videoio libraries for writing a video from
 * areaDetector cameras. A valid file path is required. Output video framerate should be set to the
 * camera framerate if a real time video is desired. Supported encodings are: H264, MPEG, DIVX, and
 * LAGS. Not all encodings will be present on each machine, thus some experimentation may be
 * required. The output video will $FILEPATH/CV_Output_Vid_$DATETIME.mp4
 *
 * @inCount     -> 4
    ...
    try {
        ...
        if (!this->isRecording && start_stop == 1) {
            ...
            if (this->filepath == "") {
                ...
                this->video = VideoWriter(output_file, fourcc, outputFramerate,
                                          Size(img.cols, img.rows), color_bool);
            }
        }
        if (this->isRecording) {
 ...
 *
 * @inCount     -> 5
 * @inFormat    -> [Num Largest Contours (Int), Blur Degree (Int), Threshold Value (Int), Upper Size
 * Threshold (Int), Lower Size Threshold (Int)]
 *
 * @outCount    -> 2-10
...
ADCVStatus_t NDPluginCVHelper::find_centroids(Mat& img, double* inputs, double* outputs) {
    ...
    try {
        ...
        for (j = 0; j < largestContours.size(); j++) {
            contour_centroids[j] = Point2f((contour_moments[j].m10 / contour_moments[j].m00),
                                           (contour_moments[j].m01 / contour_moments[j].m00));
        }
        int counter = 0;
 ...
 * Function that does feature detection on images a set number of frames apart, and attempts to
 * calculate the movement vector for the calculated key points. It uses ORB feature detection and
 * vector flow
 *
...
ADCVStatus_t NDPluginCVHelper::obj_identification(Mat& img, double* inputs, double* outputs) {
    ...
    try {
        ...
        for (k = 0; k < contours.size(); k++) {
            if (contourArea(contours[k]) > lowerSizeThreshold &&
                contourArea(contours[k]) < upperSizeThreshold) {
...
/**
 * WRAPPER  ->  Median Blur
 * Function that applies a median blur to an image. A median blur is a non-linear filter that
 * replaces each pixel with the median value of the pixels in a square kernel around it. This is
 * useful for removing noise from an image while preserving edges. The kernel size must be an odd
 * number.
 *
 * @inCount     -> 1
 ...
 * This is an unimplemented wrapper function that has already been added to the PV database in order
 * to simplify creating user defined functions. Simply implement this function and its description
 * function, and then select 'User Function' in function set 3.
 *
 * @inCount     -> n
 * @inFormat    -> [Param1 (Int), Param2 (Double) ...]
 *
 ...
 *
 * @inCount     -> 5
 * @inFormat    -> [[Distance Threshold (Int), Blur Kernel Size (Int), Threshold (Int), Apply Blur
 * (Toggle), Pixel Size Threshold (Int)]
 *
 * @outCount    -> 2
...
ADCVStatus_t NDPluginCVHelper::distance_between_ctrs(Mat& img, double* inputs, double* outputs) {
    ...
    try {
        ...
        if (contours.size() > 1) {
            ...
            if (outputs[1] < distance_threshold)
                outputs[0] = 1;
            else
                outputs[0] = 0;
            cvHelperStatus = "Finished processing distance between contours";
        } else
            cvHelperStatus = "Couldn't identify two or more contours";
        temp.release();
    } catch (Exception& e) {
        print_cv_error(e, functionName);
        status = cvHelperError;
 ...
 * Converts the format of the image to a different one for use with other AD Plugins. This is useful
 * for cameras that only support one format but a different one is required, ex. ADPluginDmtx needs
 * 8bit rgb image, so grayscale camera needs to be converted.
 *
 * @inCount     -> 2
...
ADCVStatus_t NDPluginCVHelper::convert_image_format(Mat& img, double* inputs, double* outputs) {
    ...
    int toRGB = inputs[1];
    try {
        if (toGray == 1 && img.channels() == 3)
            cvtColor(img, img, COLOR_BGR2GRAY);
        else if (toRGB == 1 && img.channels() == 1)
 ...
 * @return: void
 *
ADCVStatus_t NDPluginCVHelper::get_YOURFUNCTION_description(string* inputDesc, string* outputDesc,
string* description){ ADCVStatus_t status = cvHelperSuccess; int numInput = ?; int numOutput = ?;
 ...
 * @return: void
 */
void NDPluginCVHelper::populate_remaining_descriptions(string* inputDesc, string* outputDesc,
                                                       int nIn, int nOut) {
    int i, j;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_threshold_description(string* inputDesc, string* outputDesc,
                                                         string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 2;
    int numOutput = 0;
 ...
 *
 * @params[out]: inputDesc      -> array of input descriptions
 * @params[out]: outputDesc     -> array of output    else cvHelperStatus = "Image processed
 * successfully"; descriptions
 * @params[out]: description    -> overall function usage description
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_gaussian_blur_description(string* inputDesc, string* outputDesc,
                                                             string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 1;
    int numOutput = 0;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_laplacian_description(string* inputDesc, string* outputDesc,
                                                         string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 4;
    int numOutput = 0;
    ...
    *description =
        "Edge detection using a combination of a Gaussian Blur kernel and a Laplacian kernel";
    populate_remaining_descriptions(inputDesc, outputDesc, numInput, numOutput);
    return status;
}
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_dist_between_description(string* inputDesc, string* outputDesc,
                                                            string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 5;
    int numOutput = 2;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_sharpen_description(string* inputDesc, string* outputDesc,
                                                       string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 4;
    int numOutput = 0;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_subtract_description(string* inputDesc, string* outputDesc,
                                                        string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 1;
    int numOutput = 0;
    ...
    *description =
        "Subtracts consecutive images. Used in certain implementations of Dual Thresholding";
    populate_remaining_descriptions(inputDesc, outputDesc, numInput, numOutput);
    return status;
}
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_image_stats_description(string* inputDesc, string* outputDesc,
                                                           string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 0;
    int numOutput = 9;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_canny_edge_description(string* inputDesc, string* outputDesc,
                                                          string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 4;
    int numOutput = 8;
    ...
    *description =
        "Edge detection using the 'Canny' function. First blurs the image, then thresholds, then "
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_median_blur_description(string* inputDesc, string* outputDesc,
                                                           string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 1;
    int numOutput = 0;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_centroid_finder_description(string* inputDesc,
                                                               string* outputDesc,
                                                               string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 5;
    ...
    *description =
        "Centroid computation. Thresholds, then finds centroid. Thresholds used to remove contours "
        "by area";
    populate_remaining_descriptions(inputDesc, outputDesc, numInput, numOutput);
    return status;
}
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_movement_vectors_description(string* inputDesc,
                                                                string* outputDesc,
                                                                string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 2;
    ...
    *description =
        "Function that tracks movement of objects between two images separated by a certain number "
        "of frames. Not fully implemented";
    populate_remaining_descriptions(inputDesc, outputDesc, numInput, numOutput);
    return status;
}
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_obj_identification_description(string* inputDesc,
                                                                  string* outputDesc,
                                                                  string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 4;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_user_function_description(string* inputDesc, string* outputDesc,
                                                             string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 0;
    int numOutput = 0;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_default_description(string* inputDesc, string* outputDesc,
                                                       string* description) {
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_convert_format_descripton(string* inputDesc, string* outputDesc,
                                                             string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 2;
    int numOutput = 0;
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_video_record_description(string* inputDesc, string* outputDesc,
                                                            string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 0;
    int numOutput = 0;
    *description =
        "Video functions not built into ADCompVision. Set OPENCV_WITH_VIDEO=YES in Makefile and "
        "rebuild.";
#ifdef NDCV_WITH_VIDEO
    numInput = 4;
    ...
    *description =
        "Function that allows for recording video using areaDetector. Requires valid file path";
#endif
    populate_remaining_descriptions(inputDesc, outputDesc, numInput, numOutput);
    return status;
}
 ...
 * @return: void
 */
ADCVStatus_t NDPluginCVHelper::get_log_scaling_description(string* inputDesc, string* outputDesc,
                                                           string* description) {
    ADCVStatus_t status = cvHelperSuccess;
    int numInput = 2;
    int numOutput = 0;
...
/**
 * Function that is called from the ADCompVision plugin. It detects which function is being
 * requested, and calls the appropriate opencv wrapper function from those above.
 *
 ...
 */
ADCVStatus_t NDPluginCVHelper::processImage(Mat& image, ADCVFunction_t function,
                                            ADCVCameraDepth_t camera_depth, double* inputs,
                                            double* outputs) {
...
/**
 * This function is called from the ADCompVision plugin. It returns information regarding the input
 * types, output types, and the function itself, for display in the U.I.
 *
 ...
 */
ADCVStatus_t NDPluginCVHelper::getFunctionDescription(ADCVFunction_t function, string* inputDesc,
                                                      string* outputDesc, string* description) {
    ...
    if (function == ADCV_NoFunction)
        cvHelperStatus = "No function selected";
